...
/* 
 ...
 ec.c: API layer for Simple Open EtherCAT master
        ...
        M.J.C. Ronde & S.R. Bogeman 24 April 2012: Added EL5002 SSI interface
		M.J.C. Ronde              14 Feb. 2013: Added EL4134 and EL5152
        Arthur Ketels             20 februari 2013: Added EL1819 and EL2809, bugfix for EL4134
...
PRIVATE int ec_check_root(void);
...
/* EL5152 encoder module variable */
static int EL5152_count0_0[MAX_NUMBER_OF_SLAVES], EL5152_count0_1[MAX_NUMBER_OF_SLAVES], EL5152_ifirst[MAX_NUMBER_OF_SLAVES];
...
static int nEK1100=0, nEL1008=0, nEL1018=0, nEL1014=0, nEL1819=0,
	nEL2004=0, nEL2008=0, nEL2809=0,
	nEL3102=0, nEL3104=0, 
	nEL4132=0, nEL4134=0, nEL4038=0, 
	nEL5002=0, nEL5101=0, nEL5152=0, 
	nEbox=0;
...
int ec_EL1819_di_read_chan(double *pvalue, int ichan, int ilink)
{
        int ivalue;

        if ( (ilink<LINK_ID_LOW) || (ilink>LINK_ID_HIGH) || (ilink>nEL1819-1) ) { return EC_ERR_INVALID_LINK_ID; }
  
        if ( (ichan<0) || (ichan>15) ) { return EC_ERR_INVALID_CHAN_ID; }

        ivalue=(in_EL1819_[ilink]->inbits & bitmask16[ichan] )>>ichan;
...
int ec_EL2809_do_write_chan(double value, int ichan, int ilink)
{
        int ivalue;

        if ( (ilink<LINK_ID_LOW) || (ilink>LINK_ID_HIGH) || (ilink>nEL2809-1) ) { return EC_ERR_INVALID_LINK_ID; }
        ...
        switch (ivalue) {
        case 0:
            out_EL2809_[ilink]->outbits = (out_EL2809_[ilink]->outbits & (~bitmask16[ichan]));
            break;
        case 1:
            out_EL2809_[ilink]->outbits = (out_EL2809_[ilink]->outbits | bitmask16[ichan]);
            break;
...
int ec_EL4134_dac_write_chan(double voltage_f, int ichan, int ilink)
{
        double temp;
        int16 voltage_i;

        if ( (ilink<LINK_ID_LOW) || (ilink>LINK_ID_HIGH) || (ilink>nEL4134-1) ) { return EC_ERR_INVALID_LINK_ID; }
        ...
        switch (ichan) {
            case 0:
                    out_EL4134_[ilink]->outvalue1=voltage_i;
                    break;
            case 1:
                    out_EL4134_[ilink]->outvalue2=voltage_i;
                    break;
            case 2:
                    out_EL4134_[ilink]->outvalue3=voltage_i;
                    break;
            case 3:
                    out_EL4134_[ilink]->outvalue4=voltage_i;
                    break;                
        }
...
int ec_EL5152_enc_read_chan(double *pvalue, int ichan, int ilink)
{
        int ivalue;

        if ( (ilink<LINK_ID_LOW) || (ilink>LINK_ID_HIGH)|| (ilink>nEL5152-1) ) { return EC_ERR_INVALID_LINK_ID; }
        
        /* Module has 2 channels */
        if ( (ichan<0) || (ichan>1) ) { return EC_ERR_INVALID_CHAN_ID; }
        
        if (EL5152_ifirst[ilink] == 1) {
            EL5152_count0_0[ilink] = in_EL5152_[ilink]->channel0;
            EL5152_count0_1[ilink] = in_EL5152_[ilink]->channel1;
            EL5152_ifirst[ilink] = 0;
        }

        switch(ichan){
            case 0:
                ivalue=in_EL5152_[ilink]->channel0 - EL5152_count0_0[ilink];
                break;   
            case 1:
                ivalue=in_EL5152_[ilink]->channel1 - EL5152_count0_1[ilink];
                break;   
        }
        *pvalue=(double) ivalue;
...
PRIVATE int ec_count_ports(void) /* counts number of ports of a specific type */
{
	...
	for ( cnt=1; cnt<=ec_slavecount ;cnt++) {
        	switch (ec_slave[cnt].eep_id) {
                ...
                case EL1819_DEV_ID: /* digital in */
                        ndin=ndin+EL1819_NCHAN;
               		break;
                ...
                case EL2809_DEV_ID: /* digital out */
                        ndout=ndout+EL2809_NCHAN;
              		break;
                ...
                case EL4132_DEV_ID: /* analog out */
                        ndac=ndac+EL4132_NCHAN;
               		break;
                case EL4134_DEV_ID: /* analog out */
                        ndac=ndac+EL4134_NCHAN;
               		break;
                case EL4038_DEV_ID: /* analog out */
                ...
                case EL5101_DEV_ID: /* encoder in */
              		...
              		break;
                case EL5152_DEV_ID: /* encoder in*/
                        nenc=nenc+EL5152_NCHAN;
                    break;
...
int ec_dac_write_chan(double voltage_f, int port_id)
{
        switch ( (ppdac+port_id)->dev_id ) {
            ...
            case EL4134_DEV_ID:
                return ec_EL4134_dac_write_chan(voltage_f, (ppdac+port_id)->chan_id, (ppdac+port_id)->link_id);                
            case EBOX_DEV_ID:
        case EBOX_DEV_ID2:
...
int ec_enc_read_chan(double *pvalue, int port_id)
{
        ...
        case EL5152_DEV_ID:
                return ec_EL5152_enc_read_chan(pvalue,(ppenc+port_id)->chan_id, (ppenc+port_id)->link_id);
        case EBOX_DEV_ID:
        case EBOX_DEV_ID2:
...
int ec_din_read_chan(double *pvalue, int port_id)
{
        ...
        case EL1819_DEV_ID:  
            return ec_EL1819_di_read_chan(pvalue, (ppdin+port_id)->chan_id, (ppdin+port_id)->link_id);            
        case EL1014_DEV_ID:  
...
int ec_dout_write_chan(double pvalue, int port_id)
{
        ...
        case EL2809_DEV_ID:            
                return ec_EL2809_do_write_chan(pvalue, (ppdac+port_id)->chan_id, (ppdac+port_id)->link_id);            
        case EBOX_DEV_ID:
        case EBOX_DEV_ID2:
...
int ec_start(char* netif)
{
    ...
    printf("ec_start reports: checking ROOT permission.\n");
    if( (retval = ec_check_root()) != EC_SUCCESS){
        printf("ec_start reports: user does not have ROOT permission.\n");
        printf("please restart as ROOT, i.e. sudo ./executable \n");
        return retval;
    }
                		...
                		/*slave configuration*/
               			...
               			switch (ec_slave[cnt].eep_id) {
                                ...
                                case EL1819_DEV_ID:
                                	...
                                	in_EL1819_[nEL1819] = (in_EL1819t*) ec_slave[cnt].inputs;
                                    
                                        for (i=0; i<EL1819_NCHAN; i++) {
                                                /* din, chanel i */
                                                ec_set_din_port_capabilities(idin, nEL1819, i, 1, EL1819_DEV_ID, EL1819_DEV_STR);
                                                idin++;
                                        }
                                ...
                                case EL2809_DEV_ID:
                                	...
                                	out_EL2809_[nEL2809] = (out_EL2809t*) ec_slave[cnt].outputs;  

                                        for (i=0; i<EL2809_NCHAN; i++) {
                                                /* dout, chanel i */
                                                ec_set_dout_port_capabilities(idout, nEL2809, i, 1, EL2809_DEV_ID, EL2809_DEV_STR);
                                                idout++;
                                        }
                                ...
                                case EL4134_DEV_ID:
                                	...
                                	out_EL4134_[nEL4134] = (out_EL4134t*) ec_slave[cnt].outputs;

                                        for (i=0; i<EL4134_NCHAN; i++) {
                                                /* dac, channel i */
                                                ec_set_dac_port_capabilities(idac, nEL4134, i, EL4134_DAC_RESOLUTION, EL4134_DAC_RANGE, 1, EL4134_DEV_ID, EL4134_DEV_STR);
                                                idac++;
                                        }
                                ...
                                case EL5152_DEV_ID:
                                	...
                                	out_EL5152_[nEL5152] = (out_EL5152t*) ec_slave[cnt].outputs;
                                	in_EL5152_[nEL5152] = (in_EL5152t*) ec_slave[cnt].inputs;

                                        for (i=0; i<EL5152_NCHAN; i++) {
                                                /* enc, channel i */
                                                ec_set_enc_port_capabilities(ienc, nEL5152, i, 1, EL5152_DEV_ID, EL5152_DEV_STR);
                                                ienc++;
                                        }
...
PRIVATE int ec_setop(void)
{
	int chk;
        ...
        chk = 40; /* 40*50ms = 2000ms total timeout */
        /* wait for all slaves to reach OP state */
        do
        {
           /* keep sending processdata every 50ms as long as some slaves are not in OP */	
           ec_send_processdata();
           ec_receive_processdata(EC_TIMEOUTRET);

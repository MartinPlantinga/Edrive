...
%              March, 5th, 2004: added lowpass trigger function
...
%   if necessary restore QADScope View
    if ~strcmp(get(gcf,'Name'),'E/Scope View')
        start_scope;
        g.QS_REDRAW=1;
    end
				...
				set(h,'LineWidth',2.0);
                    ...
                    case def.QSST_TIME,
                        ...
                        if g.QS_REDRAW==1
                            ...
                            xlabel('time [s]');
                            if i<=g.QS_MAXCHAN
                                ylabel(sprintf('scope %d: [V]',j));
                                legstr=sprintf('a ch%d',i-1);
                            else
                                ylabel(sprintf('scope %d: [counts]',j));
                                legstr=sprintf('e ch%d',i-1-g.QS_MAXCHAN);
                            end
                    ...
                    case def.QSST_FFT_MOD,
                        ...
                        if g.QS_REDRAW==1
                            ...
                            xlabel('freq [Hz]');
                            if i<=g.QS_MAXCHAN
                                ylabel(sprintf('scope %d: [V/Hz]',j));
                                legstr=sprintf('a ch%d',i-1);
                            else
                                ylabel(sprintf('scope %d: [counts/Hz]',j));
                                legstr=sprintf('e ch%d',i-1-g.QS_MAXCHAN);
                            end
                            thelegend=[thelegend; legstr];                       
                        end
                    ...
                    case def.QSST_ESD_AUTO,
                        ...
                        if g.QS_REDRAW==1
                            ...
                            if i<=g.QS_MAXCHAN
                                ylabel(sprintf('scope %d: [V^2s/Hz in dB]',j));
                                legstr=sprintf('a ch%d',i-1); 
                            else
                                ylabel(sprintf('scope %d: [counts^2s/Hz in dB]',j));
                                legstr=sprintf('e ch%d',i-1-g.QS_MAXCHAN); 
                            end
                            thelegend=[thelegend; legstr];
                        end
			    ...
			    ttt(2:end)=2*data.PSD_AUTO(i,2:end);
                    ...
                    case def.QSST_PSD_AUTO,
                        ...
                        if g.QS_REDRAW==1
                            ...
                            set(gca,'XLim',[f2(1) f2(end)]);
                            if i<=g.QS_MAXCHAN
                                ylabel(sprintf('scope %d: [V^2/Hz in dB]',j));
                                legstr=sprintf('a ch%d',i-1); 
                            else
                                ylabel(sprintf('scope %d: [counts^2/Hz in dB]',j));
                                legstr=sprintf('e ch%d',i-1-g.QS_MAXCHAN); 
                            end
                            thelegend=[thelegend; legstr];
                        end
			    ...
			    ttt(2:end)=2*data.PSD_AUTO(i,2:end);
                    ...
                    case def.QSST_PSD_AUTO_CUM,
                        ...
                        if g.QS_REDRAW==1
                            ...
                            set(gca,'XLim',[f2(1) f2(end)]);
                            if i<=g.QS_MAXCHAN
                                ylabel(sprintf('scope %d: [V^2 in dB]',j));
                                legstr=sprintf('a ch%d',i-1); 
                            else
                                ylabel(sprintf('scope %d: [counts^2 in dB]',j));
                                legstr=sprintf('e ch%d',i-1-g.QS_MAXCHAN); 
                            end
                            thelegend=[thelegend; legstr];
                        end
                            ...
                            if g.QS_REDRAW==1
                                ...
                                if g.QS_IN<=g.QS_MAXCHAN
                                    if g.QS_OUT<=g.QS_MAXCHAN
                                        ylabel(sprintf('scope %d: [V^2/Hz in dB]',j));
                                        legstr=sprintf('a ch%d/a ch %d',g.QS_OUT-1,g.QS_IN-1);
                                    else
                                        ylabel(sprintf('scope %d: [V*counts/Hz in dB]',j));                                    
                                        legstr=sprintf('e ch%d/a ch %d',g.QS_OUT-1-g.QS_MAXCHAN,g.QS_IN-1);
                                    end
                                else
                                    if g.QS_OUT<=g.QS_MAXCHAN
                                        ylabel(sprintf('scope %d: [counts*V/Hz in dB]',j));
                                        legstr=sprintf('a ch%d/e ch %d',g.QS_OUT-1,g.QS_IN-1-g.QS_MAXCHAN);
                                    else
                                        ylabel(sprintf('scope %d: [counts^2/Hz in dB]',j));                                    
                                        legstr=sprintf('e ch%d/e ch %d',g.QS_OUT-1-g.QS_MAXCHAN,g.QS_IN-1-g.QS_MAXCHAN);
                                    end
                                end
                                thelegend=[thelegend; legstr];                    
                            end
                    ...
                    case def.QSST_FRF_MOD,
                        if i==g.QS_IN                     
                            if g.QS_REDRAW==1
                                ...
                                if g.QS_IN<=g.QS_MAXCHAN
                                    if g.QS_OUT<=g.QS_MAXCHAN
                                        ylabel(sprintf('scope %d: [- in dB]',j));
                                        legstr=sprintf('a ch%d/a ch %d',g.QS_OUT-1,g.QS_IN-1);
                                    else
                                        ylabel(sprintf('scope %d: [counts/V in dB]',j));                                    
                                        legstr=sprintf('e ch%d/a ch %d',g.QS_OUT-1-g.QS_MAXCHAN,g.QS_IN-1);
                                    end
                                else
                                    if g.QS_OUT<=g.QS_MAXCHAN
                                        ylabel(sprintf('scope %d: [V/counts in dB]',j));
                                        legstr=sprintf('a ch%d/e ch %d',g.QS_OUT-1,g.QS_IN-1-g.QS_MAXCHAN);
                                    else
                                        ylabel(sprintf('scope %d: [- in dB]',j));                                    
                                        legstr=sprintf('e ch%d/e ch %d',g.QS_OUT-1-g.QS_MAXCHAN,g.QS_IN-1-g.QS_MAXCHAN);

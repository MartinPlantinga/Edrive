...
%   20120405 - Added 'hold' use: 'hold on' and 'hold off' between calls of frf (Rob Hoogendijk)
...
%settings (available if setplot.m is installed)
fontname='times';           %of the labels, axes, legends
fontsize=10;                %of the labels, axes, legends
rowspacing=[];              %in centimeter, [] gives minimal spacing
colspacing=[];              %in centimeter, [] gives minimal spacing
defaultwrapsetting='off';   %wrap the phase to a range -180,180 or not
...
try wrapping = options{7};   catch, wrapping=defaultwrapsetting; end;
...
try %try to read handles from userdata of figure to enable subsequent plotting    
    ...
    replaceplots=strcmp(get(gca,'nextplot'),'replace');
    if(replaceplots)
        clf;error;
    end
...
if exist('setplot','file')==2
    setplot([], [], {0.5,1.0,rowspacing,1.1,1.1,colspacing}, fontsize, fontname);
else
...
%% private functions
function phase=wrap(phase,top)
if nargin == 1,
    phase = phase-(round(phase/360)*360);
elseif nargin == 2,
    f	= f+180-top;
    fw	= f-(round(f/360)*360)-180;
end
end
